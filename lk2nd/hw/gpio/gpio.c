...
int gpio_config(uint32_t nr, uint32_t flags)
{
	...
	default:
		dprintf(CRITICAL, "gpio_config: device %d is not known.", NR_DEV(nr));
	}
...
void gpio_set(uint32_t nr, uint32_t on)
{
	...
	default:
		dprintf(CRITICAL, "gpio_set: device %d is not known.", NR_DEV(nr));
	}
}
...
int gpio_get(uint32_t nr)
{
	...
	default:
		dprintf(CRITICAL, "gpio_get: device %d is not known.", NR_DEV(nr));
	}
